(ns neuralink_api.main
  "Kotodama WASM entrypoint for the Neuralink_api clean-room actor (L4) — Clojure port.
  ...
  facts (`neuralink_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "neuralink_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "neuralin_dev"
    :fields [:model :channels :samplingHz :firmware]     :required [:model :channels]
    :coerce {:channels :int :samplingHz :int} :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "neuralin_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"neuralink_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "neuralink_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :neuralink_api-compat/ready)
